...
(function () {
    ...
    // ВАЖНО: angularVelocity в Matter.js — радианы за ШАГ (1/60 c), а не за секунду.
    // Было 0.4 (≈4 оборота/сек) — нереально быстро, поэтому сторож НЕ ловил реальные случаи
    // (баг с вращением кота в углу был виден на видео, но в телеметрию не попадал).
    // 0.05 рад/шаг ≈ пол-оборота в секунду — уже хорошо заметное вращение.
    var SPIN_ANGVEL = 0.05;            // рад/шаг — заметное вращение
    var SPIN_MOVE_PX = 12;             // почти не сдвинулся (с запасом на дрожание)
